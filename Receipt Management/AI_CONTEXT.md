# AI Context Document: VoucherOCR Web Platform — Historical Reference

**Historical reference only:** This document describes the separate web-based VoucherOCR platform. It is useful for understanding the expected OCR, validation, batch, supplier, and ML-feedback concepts, but it is **not** the primary context for the new Android FarmAI application.

**Android source of truth:** For Android app development, read `App AI Context.md` first. For broader Android roadmap details, read `BROADER_SCOPE_PLANNER.md`.
